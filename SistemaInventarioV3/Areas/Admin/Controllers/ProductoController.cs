...
namespace SistemaInventarioV3.Areas.Admin.Controllers
{
    [Area("Admin")]
    public class ProductoController : Controller
    {
        ...
        public async Task<IActionResult> Upsert(ProductoVM productoVM)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    //Actualizar
                    var objProducto = await _unidadTrabajo.Producto.ObetenerPrimero(p => p.Id == productoVM.Producto.Id, isTracking: false);
                    if (files.Count > 0)
                    {
                        string upload = webRootPath + DS.ImagenRuta;
                        string fileName = Guid.NewGuid().ToString();
                    ...
                    //Caso no se carga una nueva imagen
                    else
                    {
            ...
            public async Task<IActionResult> Delete(int id)
            {
                ...
                if (productoDb == null)
                {
                    return Json(new { succes = false, message = "Error producto no encontrada" });
                }
